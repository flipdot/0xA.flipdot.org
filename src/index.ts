...
function makeStars(container) {
  ...
  draw.viewbox(0, 0, width / 3, height / 3);
  drawStars(draw);
}
...
function positionRandomly(draw, obj, paddingX = 0, paddingY = 0) {
  ...
  obj
    .x(randomNumber(bbox.w / 2 + paddingX, viewbox.width - bbox.w / 2 - paddingX))
    .y(randomNumber(bbox.h / 2 + paddingY, viewbox.height - bbox.h / 2 - paddingY));
}
...
function drawRocket(draw) {
  ...
  rocket.cy(0);
  const yRange = height * rocketScale * 2;
  const xRange = width * rocketScale * 2;
  rocket.cy(randomNumber(height / 2 - yRange / 2, height / 2 + yRange / 2) + 50);
  rocket.cx(randomNumber(width / 2 - xRange / 2, width / 2 + xRange / 2));
